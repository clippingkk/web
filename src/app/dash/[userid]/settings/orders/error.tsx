...
import { AlertTriangle, RefreshCw, ExternalLink, HelpCircle, AlertCircle } from 'lucide-react'
      ...
      <div className="flex flex-col items-center text-center">
        ...
        {parsedError ? (
            ...
            <div className="bg-gray-50 dark:bg-gray-800/60 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
              ...
              <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center bg-white/50 dark:bg-gray-700/30">
                <div className="flex items-center">
                  ...
                  <span className="font-medium text-gray-900 dark:text-white">{parsedError.code || 'Error'}</span>
                </div>
                {parsedError.status && (
              ...
              <div className="p-4 text-left">
                ...
                <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                  {parsedError.param && (
                    <div className="flex"><span className="font-medium w-24">Parameter:</span> {parsedError.param}</div>
                  )}
                  {parsedError.request_id && (
                    <div className="flex"><span className="font-medium w-24">Request ID:</span> {parsedError.request_id}</div>
                  )}
                  {parsedError.type && (
                    <div className="flex"><span className="font-medium w-24">Error type:</span> {parsedError.type}</div>
                  )}
                </div>
              </div>
          ...
          <p className="text-gray-600 dark:text-gray-300 mb-6">
            {error?.message || t('app.error.default', 'We encountered an error while loading your order history.')}
          </p>
        )}
