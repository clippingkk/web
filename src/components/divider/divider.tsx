...
function Divider({ title, variant = 'elegant', color = 'blue' }: TDividerProps) {
  const colorClasses = {
    blue: {
    ...
    return (
      <div className="relative flex items-center w-full my-12">
        ...
        <div className="flex-grow relative h-px">
          ...
          <div className={`absolute inset-0 bg-gradient-to-r ${colors.accent} animate-pulse`} style={{ animationDuration: '3s' }}></div>
        </div>
        
        {/* Enhanced title container */}
        <div className="relative mx-6">
          {/* Subtle glow effect */}
          <div className={`absolute inset-0 blur-xl ${colors.bg} opacity-60`}></div>
          
          {/* Main title container */}
          <div className={`relative px-6 py-3 ${colors.bg} backdrop-blur-sm rounded-2xl border ${colors.border} shadow-sm ${colors.glow}`}>
            {/* Decorative dots */}
            <div className="flex items-center gap-2">
              <div className={`w-1.5 h-1.5 rounded-full bg-gradient-to-r ${colors.accent.replace('/30', '/60').replace('/40', '/80')}`}></div>
              <span className={`text-base font-semibold ${colors.text} tracking-wide`}>
                {title}
              </span>
              <div className={`w-1.5 h-1.5 rounded-full bg-gradient-to-r ${colors.accent.replace('/30', '/60').replace('/40', '/80')}`}></div>
            </div>
        ...
        <div className="flex-grow relative h-px">
          <div className="absolute inset-0 bg-gradient-to-r from-transparent via-gray-300 dark:via-zinc-700 to-transparent"></div>
          <div className={`absolute inset-0 bg-gradient-to-r ${colors.accent} animate-pulse`} style={{ animationDuration: '3s', animationDelay: '1s' }}></div>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="relative flex items-center w-full my-8">
      ...
      <span className={`flex-shrink mx-4 px-4 py-2 text-lg font-medium ${colors.text} ${colors.bg} rounded-xl shadow-sm border ${colors.border} backdrop-blur-sm`}>
        {title}
      </span>
      <div className="flex-grow border-t border-gray-300 dark:border-zinc-700"></div>
    </div>
